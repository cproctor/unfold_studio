...
> I want to add a new feature: support for Anthropic-style LLM apis in addition to OpenAI-style apis. 
> This will require an additional backend (`unfold_studio/text_generation/backends.py`; create a new proposed issue 
> to handle this), and consequently perhaps it will make sense to rename the `OPENAI_API_KEY` setting (or maybe 
> to avoid ambiguity, we allow `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` (or something), and different backends 
> expect different keys to be provided. We also define TEXT_GENERATION as a dict in settings, sort of like how 
> DATABASES is specified--is this not used? I prefer this style, which would take care of provider-agnostic setting key. 
...
> Public stories should not be shareable in this way--too much potential for mischief.
> Do ensure that an appropriate error page is displayed in the embed when an ineligible story is specfied. 
> I like the Unfold Studio chrome--use the logo, hyperlinked back to the base url.
...
> Injecting styled HTML is not a docuemnted feature, but some authors have discovered that this is possible, 
> and have created interesting effects with it. Is there a clean way of allowing html tags and style attributes, 
> while disabling all js? This would include scripts, event handlers, and other means of code injection. 
...
> Add a crontab to run this regularly to the deployment instructions.
...
> Ensure that models which define custom managers will properly inherit `get_queryset` from this manager.
...
> 
> Response: There is no need to automatically purge at this point--there isn't any soft-deleted
> data being created regularly. The delete_user command should ensure that all traces of the user
> are removed (for compliance reasons). If we can keep anonymous literacy events, explain how this would work.
...
> Create a new task in this list: Create sphinx docs for Unfold Studio, with an internal dev audience. 
> (We already have separate docs for nontechnical users in `~/Repos/unfold_studio_static/site/content`)
> Move all internal documentation there, including sections for: 
> - Deployment
> - dev deployment
> - dev quickstart, providing an introduction to the codebase structure. 
...
> Which do you prefer?
> 
> Response: Let's go with Option A; I want to plan for future scale. But story authors will be 
> implicitly compiling their stories whenever they save, and expecting the story to run. Surely
> 2 seconds of latency will feel too long? 
...
> give more details on the deployment runbook. I will want a section of the docs to address deployment, 
