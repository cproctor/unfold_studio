...
> Agreed. Drop the long-term goal, and revise this to focus on the short-term fix. 
...
> Response: Actually, just remove the capacity for public stories all togehter--this will no longer be part of Unfold Studio, because the use case (workshop participants needing to immediately try
> out Unfold Studio without having to create an account) can be readily satisfied using join codes. Remove public stories everywhere: in mixins, in model managers, in routes, etc. 
