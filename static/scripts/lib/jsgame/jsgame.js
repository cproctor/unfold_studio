...
JSGame.prototype = {
    ...
    wait_for_text: function(options) {
        ...
        this.valid_text_options = _.reduce(options || [], function(memo, option) {
            memo.push((option instanceof Array) ? option[0] : option)
            return memo
        }, []);
        var options_html = _.reduce(options, function(html, option) {
            if (option instanceof Array) {
                var optionText = "" + option[0] + ": " + option[1]
            }
            else {
                var optionText = option
            }
            return html + "<li>" + optionText + "</li>";
        }, "<p>Options</p><ul>") + "</ul>";
